...
if (isset($_GET['data'])) {
 ...
 $sql_dkategori_buku = "SELECT `kategori_buku` FROM  `kategori_buku` WHERE `id_kategori_buku` = $id_kategori";
 $query              = mysqli_query($koneksi, $sql_dkategori_buku);
 $data_kbuku         = mysqli_fetch_row($query);
 $kategori           = $data_kbuku[0];
...
<section id="katalog-item">
  <main role="main" class="container">
    <h2 class="text-primary">CATEGORIES: <?php echo $kategori ?></h2><br><br>
    <div class="row">
...
$sql_b = "SELECT `b`.`id_buku`, `b`.`judul`, `b`.`cover`,
            ...
            `p`.`id_penerbit` WHERE `id_kategori_buku`= $id_kategori
